...
from custom_ml_library.utils import euclidean_distance
...
class TestEuclideanDistance:
    ...
    def test_raises_error_for_non_1d_inputs(self):
        ...
        # point1_pl = pl.Series( # This variable was unused.
        #     [pl.Series([1, 2]), pl.Series([3, 4])]
        # )
    ...
    def test_raises_error_for_invalid_types(self):
        ...
        point2 = np.array([4, 5, 6])
        with pytest.raises(
            TypeError, match="Inputs must be Polars Series or NumPy ndarrays."
        ):
            euclidean_distance(point1, point2)
    ...
    def test_zero_length_input(self):
        ...
        np.testing.assert_almost_equal(
            euclidean_distance(point1, point2), expected_distance
        )

        point1_np = np.array([])
        point2_np = np.array([])
        np.testing.assert_almost_equal(
            euclidean_distance(point1_np, point2_np), expected_distance
        )
