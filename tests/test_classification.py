...
class TestKNeighborsClassifier:
  ...
  def sample_data(self):
    ...
    X = pl.DataFrame({
        "feature1": [1, 1.5, 2, 5, 5.5, 6, 1.2, 5.2],
        "feature2": [1, 1.5, 1, 4, 4.5, 4, 0.8, 4.2]
    })
  ...
  def test_fit_and_predict(self, sample_data):
    ...
    new_points = pl.DataFrame({
        "feature1": [1.1, 5.1, 0.9, 6.1],
        "feature2": [1.1, 4.1, 1.2, 3.8]
    })
    expected_new_predictions = pl.Series([0,1,0,1])
    predictions_new = model.predict(new_points)
    np.testing.assert_array_equal(predictions_new.to_numpy(), expected_new_predictions.to_numpy())
  ...
  def test_predict_before_fit_raises_error(self, sample_data):
    ...
    with pytest.raises(RuntimeError, match="This KNeighborsClassifier instance is not fitted yet."):
      model.predict(X)
