...
from custom_ml_library.regression import LinearRegression
...
class TestLinearRegression:
    ...
    def test_fit_intercept_true(self):
        ...
        # y = 2*X1 + 3*X2 + 5
        y_np = (
            2 * X_pl.select("X1").to_series().to_numpy()
    ...
    def test_predict_before_fit_raises_error(self):
        ...
        with pytest.raises(
            RuntimeError, match="This LinearRegression instance is not fitted yet."
        ):
            model.predict(X_pl)
    ...
    def test_collinear_features(self):
        ...
        if model.coef_ is not None:  # Should be set
            ...
            np.testing.assert_almost_equal(
                model.coef_[0] + 2 * model.coef_[1], 3.0, decimal=5
            )
